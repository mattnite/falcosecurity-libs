...
bool read_one_cgroup_val(const std::string &path, std::istream &stream, int64_t &out)
{
	...
	if(str_val == "max")
	{
		g_logger.format(FALCO_LOG_SEV_DEBUG, "(cgroup-limits) value of %s is set to max, ignoring",
				path.c_str(), val);
		return false;
	}
	try
	...
	{
		g_logger.format(FALCO_LOG_SEV_DEBUG, "(cgroup-limits) value of %s (%lld) out of range, ignoring",
				path.c_str(), val);
		return false;
	}
	out = val;
...
bool get_cgroup_resource_limits(const cgroup_limits_key& key, cgroup_limits_value& value, bool name_check)
{
	...
	{
		g_logger.format(FALCO_LOG_SEV_INFO, "(cgroup-limits) mem cgroup for container [%s]: %s/%s -- no per-container memory cgroup, ignoring",
				key.m_container_id.c_str(), memcg_root->c_str(), key.m_mem_cgroup.c_str());
	}
	else
	{
		g_logger.format(FALCO_LOG_SEV_DEBUG, "(cgroup-limits) mem cgroup for container [%s]: %s/%s",
				key.m_container_id.c_str(), memcg_root->c_str(), key.m_mem_cgroup.c_str());
	...
	{
		g_logger.format(FALCO_LOG_SEV_INFO, "(cgroup-limits) cpu cgroup for container [%s]: %s/%s -- no per-container CPU cgroup, ignoring",
				key.m_container_id.c_str(), cpucg_root->c_str(), key.m_cpu_cgroup.c_str());
	}
	else
	{
		g_logger.format(FALCO_LOG_SEV_DEBUG, "(cgroup-limits) cpu cgroup for container [%s]: %s/%s",
				key.m_container_id.c_str(), cpucg_root->c_str(), key.m_cpu_cgroup.c_str());
		if(cpu_version == 2)
		{
	...
	{
		g_logger.format(FALCO_LOG_SEV_DEBUG, "(cgroup-limits) cpuset cgroup for container [%s]: %s/%s -- no per-container cpuset cgroup, ignoring",
				key.m_container_id.c_str(), cpuset_root->c_str(), key.m_cpuset_cgroup.c_str());
	}
	else
	{
		g_logger.format(FALCO_LOG_SEV_DEBUG, "(cgroup-limits) cpuset cgroup for container [%s]: %s/%s",
				key.m_container_id.c_str(), cpuset_root->c_str(), key.m_cpuset_cgroup.c_str());
