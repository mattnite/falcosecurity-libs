...
{
	enum falco_log_severity sev;
	switch(level)
	{
	case LIBBPF_WARN:
		sev = FALCO_LOG_SEV_WARNING;
		break;
	case LIBBPF_INFO:
		sev = FALCO_LOG_SEV_INFO;
		break;
	case LIBBPF_DEBUG:
		sev = FALCO_LOG_SEV_DEBUG;
		break;
	default:
		return -EINVAL;
	}
...
int pman_init_state(falco_log_fn log_fn, unsigned long buf_bytes_dim, uint16_t cpus_for_each_buffer,
